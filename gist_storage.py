...
---------------------------------------------------------------------
 ...
 Solution:
   ...
   - Not for bulk data (CSVs) — those are rebuilt on Render via
     /trigger/download (post-market) and /trigger/seed-gap (09:20 IST).

 Files backed up:
   * live_config.json    (winning config from param_sweep / allocator)
   * state.json          (open positions, halts, kill switch — via state_store)
   * strategy_ledger.json (EOD trade ledger for the allocator)
