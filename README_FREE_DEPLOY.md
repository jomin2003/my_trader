...
cron-job.org (free)          Render free web service         Dhan / Telegram
     ...
     │─────────────────────────────>│  Dhan token via TOTP         │
     │                              │                              │
     │  Sun 19:00 IST               │  GitHub Actions              │
     │  (NOT a cron endpoint)       │  .github/workflows/          │
     │                              │  weekly_opt.yml              │
     │                              │  sweep + backtest + promote  │
     │                              │  Backs up config to Gist  ────┴──> GitHub Gist
```

> The weekly optimization does **not** go through cron-job.org — there is no
> `/trigger/weekly` endpoint. It runs as the `weekly_opt.yml` GitHub Actions
> workflow and promotes tuned params to the Gist, which Render restores at boot.
...
| **State** (`state.json`) | GitHub Gist (restored only if dated today) | Open positions, halts, cooldowns survive cold starts |
| **Ledger** (`strategy_ledger.json`) | GitHub Gist | Per-strategy history feeds the adaptive allocator |
...
Sign up at **https://cron-job.org**. Create **8 jobs** as follows.
...
| 5 | `seed_gap` | `https://YOUR_URL/trigger/seed-gap` | **Mon-Fri 09:20** (`20 9 * * MON-FRI`) — writes today's gap rows so GapFill/GapGo can trade |
| 6 | `download_data` | `https://YOUR_URL/trigger/download` | **Mon-Fri 15:35** (`35 15 * * MON-FRI`) |
| 7 | `report` | `https://YOUR_URL/trigger/report` | **Mon-Fri 15:40** (`40 15 * * MON-FRI`) |
| 8 | `learn` | `https://YOUR_URL/trigger/learn` | **Mon-Fri 15:45** (`45 15 * * MON-FRI`) |

> **Weekly optimization** does **not** run on Render — it runs as the GitHub
> Actions workflow `.github/workflows/weekly_opt.yml` (Sunday 19:00 IST). That
> job promotes tuned params to the Gist, which Render restores on next boot.
> There is no `/trigger/weekly` endpoint.
>
> **`/trigger/promote` is manual-only** — it promotes the latest local
> `sweep_*.csv` via `live_config.py`. There is no cron job for it (Render's
> ephemeral disk never holds sweep files). Call it by hand after a local
> sweep, or let `weekly_opt.yml` handle promotion on Sundays.
>
> The full job list is also kept as [`external_cron_config.md`](external_cron_config.md).
...
Option A (simplest) — seed post-market data manually via browser. The secret must go in the
`X-Cron-Secret` header (query strings leak into logs), so use curl or a browser extension:
```
curl -H "X-Cron-Secret: <your_CRON_SECRET>" https://YOUR_URL/trigger/download
```
This re-downloads bars and refreshes `ob_data.csv` + `gap_data.csv`. Wait for
`✅ Post-market data + OB zones + gap table refreshed` on Telegram.
