"""Precompute Gap metadata per (symbol, date). Keeps gaps 0.25%-3.0% (fade<0.6%, go>1%)."""
...
def main():
    ...
    if not all_rows:
        # No qualifying gaps is a normal market condition, not a pipeline
        # failure. Keep any existing table rather than wiping it.
        log.warning("No tradeable gaps (0.25-3%) found today — keeping existing gap_data.csv if present.")
        return
