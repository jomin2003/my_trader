...
Telegram commands:
  ...
  /mlstatus, /models, /config, /decision SYMBOL -> ML diagnostics
  /help    -> list commands

Endpoints:
  GET  /                    - status page
  GET  /healthz             - keep-alive ping (Render health check)
  GET  /readyz              - readiness probe (Docker HEALTHCHECK)
  ...
  POST /trigger/seed-gap    - 09:20 IST: write today's gap rows (GapFill/GapGo)
  POST /trigger/download    - append bars + refresh OB data
  POST /trigger/promote     - promote latest sweep (manual / CI use)
...
try:
    ...
    logging_config.install()  # redact secrets (tokens, chat ids) from every log line
except Exception:
    pass
...
def tg_send_to(chat_id: str, text: str, silent: bool = False):
...
def tg_send(text: str, silent: bool = False):
    ...
    if TG_CHAT_ID:
        tg_send_to(TG_CHAT_ID, text, silent=silent)
...
def require_cron_secret(f):
    ...
    def _wrap(*a, **kw):
        ...
        # FIX: the secret used to be accepted via ?secret= query param, which leaks into
        # cron-site logs, proxies and access logs. Header only now.
...
def telegram_webhook():
    ...
    try:
        ...
        # FIX: with TG_CHAT_ID unset the old check let ANY Telegram user run /stop,
        # /resume (clears the circuit breaker) and /report (leaks positions/P&L) on a
        # public endpoint. Commands now require the chat to be the configured one.
        if not TG_CHAT_ID or chat_id != str(TG_CHAT_ID):
            return jsonify({"ok": True})
...
@app.route("/readyz")
def readyz():
    """Readiness probe (used by the Docker HEALTHCHECK): fails if critical deps/config bad."""
    try:
        from health_service import readiness
        body, code = readiness(base_dir=str(BASE_DIR))
        return jsonify(body), code
    except Exception as e:
        return jsonify({"status": "not_ready", "problems": [str(e)]}), 503
...
def metrics():
    ...
    try:
        ...
        return prometheus_text(), 200, {"Content-Type": "text/plain; version=0.0.4"}
    except Exception as e:
        return f"# metrics error: {e}", 500

@app.route("/metrics/snapshot")
...
@app.route("/trigger/seed-gap", methods=["POST", "GET"])
@require_cron_secret
def trigger_seed_gap():
    "Cron ~09:20 IST: seed today's gap rows so GapFill/GapGo can trade intraday."
    if STATE.get("seed_gap_running"):
        return jsonify({"ok": True, "note": "gap seed already running"})
    threading.Thread(target=_seed_gap_worker, daemon=True).start()
    return jsonify({"ok": True, "note": "gap seed started in background"})

def _seed_gap_worker():
    try:
        STATE["seed_gap_running"] = True
        t = now_ist().time()
        if not (MARKET_OPEN <= t <= dtime(11, 30)):
            return  # gaps only meaningful near the open; skip later runs
        import intraday_pattern_scanner_v2 as scn
        import seed_today_gap
        import multi_strategy_live as msl
        n = seed_today_gap.seed_today(get_dhan(), _get_universe(scn))
        msl.reload_tables()
        STATE["last_gap_seed"] = now_ist().isoformat()
        if n:
            tg_send(f"🌱 Gap seed: {n} row(s) for today", silent=True)
    except Exception as e:
        _record_error(f"seed-gap: {e}")
        tg_send(f"⚠️ Gap seed FAILED: {e}")
    finally:
        STATE["seed_gap_running"] = False
...
def trigger_download():
    def _dl():
        try:
            ...
            rc3 = _run_module("precompute_gapfill.py", [
                "--csv-dir", str(DATA_DIR),
                "--out", str(BASE_DIR / "gap_data.csv")], timeout=300)
            STATE["last_download"] = now_ist().isoformat()
            if rc1 != 0 or rc2 != 0 or rc3 != 0:
                tg_send(f"⚠️ Data pipeline: dl_rc={rc1}, ob_rc={rc2}, gap_rc={rc3}")
            else:
                tg_send("✅ Post-market data + OB zones + gap table refreshed", silent=True)
        except Exception as e:
