...
# Signal ranking: Concretum's ORB study found relative volume is the dominant
# selection filter; the default ranks by score first, RVOL as tie-break.
RANK_KEYS            = ("score", "vol_ratio", "strength")
...
# =====================================================================
# COSTS & INTRA-BAR EXIT — cost formula lives in cost_model.py (single source)
# =====================================================================
from cost_model import apply_costs as _shared_apply_costs


def _apply_costs(entry_px, exit_px, qty, side):
    # Harness keeps full precision (no paise rounding); formula identical to scanner's.
    return _shared_apply_costs(entry_px, exit_px, qty, side,
...
def backtest(symbol_bars: dict[str, pd.DataFrame],
    ...
    for bar_ts in all_ts:
        ...
        ranked = (pd.DataFrame(sigs)
                  .sort_values(list(RANK_KEYS), ascending=[False] * len(RANK_KEYS))
        ...
        for _, sig in ranked.iterrows():
            ...
            # PATCHED: pass the strategy so exits match the LIVE per-strategy
            # profiles (strategy_exits). Previously fell back to globals, so
            # the harness tested RR=2.0 while live trades RR=1.5.
            sl, tgt, sl_dist = scn.compute_sl_target(
                entry_px, direction, sig.get("atr"), strategy=sig.get("strategy"))
