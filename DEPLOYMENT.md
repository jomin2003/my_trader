...
### `app.py` — config validation at boot (consolidated)
Configuration lives in `config_registry.py` (single source of truth; the old
Phase-3 `config_loader`/`settings` stack was removed in the 2026-09-23
cleanup). Near the top of `boot_restore()`:
```python
import config_registry as cfg
log.info(f"config_version={cfg.version()}")
```
Readiness is served by `health_service.readiness()`, which validates against
`config_registry` directly.
...
### Exit authority (live path)
Exits are layered: `compute_sl_target` → `kronos_exits.adjust_exits()` →
optional `rr_predictor` override (RR gate off by default). The old typed
exit-policy V2 stack (`exit_policy.py` / `exit_models.py` /
`exit_validation.py`) was never wired into the live path and was removed in
the 2026-09-23 cleanup; `EXIT_POLICY_V2_ENABLED` no longer does anything.
...
| 2 | _(removed 2026-09-23)_ exit-policy V2 stack deleted; skip | — | — |
...
| New env vars | — | `AUDIT_ENABLED, APP_PROFILE, CONFIG_DIR, PERSISTENCE_BACKEND, PORTFOLIO_RISK_ENABLED, USE_SHARED_ENGINE, SCAN_WORKERS` |
