...
class TokenConfigError(RuntimeError):
    """Raised for missing configuration/dependencies (never SystemExit — that
    would escape callers' `except Exception` and kill the host process)."""
...
def _current_totp() -> str:
    if pyotp is None:
        raise TokenConfigError("pyotp not installed. Run: pip install pyotp")
    secret = os.getenv("DHAN_TOTP_SECRET", "").strip().replace(" ", "")
    if not secret:
        raise TokenConfigError("DHAN_TOTP_SECRET env var is not set.")
    try:
        return pyotp.TOTP(secret).now()
    except Exception as e:
        raise TokenConfigError(f"TOTP generation failed: {e}")
...
def _generate_via_totp() -> TokenState:
    ...
    if not client_id or not pin:
        raise TokenConfigError("DHAN_CLIENT_ID and DHAN_PIN env vars required.")
