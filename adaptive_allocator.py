...
# FIX: use canonical names from config_registry.StrategyName (OB_SHORTS not OB_SHORT,
# plus new VWAP arms). get_weight() normalizes so old tags still resolve.
try:
    from config_registry import StrategyName as _SN
    STRATEGIES = list(_SN.ALL)
except Exception:
    STRATEGIES = ["OB_SHORTS", "ORB", "GAPFILL", "CANDLE_STRUCT", "VWAP_RECLAIM", "VWAP_PULLBACK"]
...
def get_weight(strategy: str, base_dir: str | None = None) -> float:
    ...
    try:
        from config_registry import StrategyName as _SN
        strategy = _SN.normalize(strategy)
    except Exception:
        pass
    cfg = _load_config()
