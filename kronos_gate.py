...
STATUS (2026-09-23): the GitHub Actions forecaster that publishes
kronos_forecast.json does NOT exist in this repo yet — the reader above is
live but no producer writes the file. Until a forecaster is added,
get_view() returns None and kronos_check() is a fail-open no-op
("(True, 0, 'kronos:na')"), even in strict mode. The gate does nothing
today; wire a producer before relying on it.
"""
...
def _stale(view: dict) -> bool:
    ...
    try:
        ...
        # FIX: a naive timestamp (no tz) used to raise TypeError → caught → "not stale",
        # so a timezone-less forecast was trusted forever. Coerce to IST instead.
        if ts_dt.tzinfo is None:
            ts_dt = ts_dt.replace(tzinfo=IST)
        age_min = (datetime.now(IST) - ts_dt).total_seconds() / 60.0
        return age_min > MAX_STALE_MIN
    except Exception:
        return True  # FIX: unparseable timestamps must fail CLOSED, not be trusted forever
